...
const StripeCheckout = ({history}) => {
    ...
    const handleChange = async (e) => {
        ...
        // and display any errors as the customer inputs card details
        setDisabled(e.empty);
        ...
        <>
            {!succeeded && (<div>
                    {coupon && totalAfterDiscount !== undefined ? (
                        <p className="alert alert-success">{`Total after discount: EUR ${totalAfterDiscount}`}</p>
                    ) : (
            ...
            <div className="text-center pb-5">
                <Card 
                    ...
                    actions={[
                        <>
                            <EuroOutlined className="text-info" /><br/>Total: EUR {cartTotal}
                        </>,
                        <>
                            <CheckOutlined className="text-info" /><br/>Total payable: EUR {(payable/100).toFixed(2)}
                        </>
                    ]}
                />
            </div>
